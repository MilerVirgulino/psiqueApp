...
const PrintRecordScreen = ({ navigation }) => {
...
useEffect(() => {
    ...
    const filtered = pacientes
        ...
        .sort((a, b) => {
            // Primeiro, comparar os nomes
            const nameComparison = a.name.localeCompare(b.name);
            if (nameComparison !== 0) return nameComparison; 

            // Se os nomes forem iguais, comparar as datas (YYYY-MM-DD já permite comparação direta)
    ...
    const handleOptionShow = async (id) => {
        await AsyncStorage.setItem("idRecord", id);
            ...
            {filteredPacientes.length > 0 ? (
                filteredPacientes.map((paciente) => (
                    <TouchableOpacity key={paciente.id} onPress={() => handleOptionShow(paciente.id)} style={styles.pacienteItem}>
                        ...
                        <Text style={styles.pacienteNome}>{"Data:  " + formatDate(paciente.date)  || "Nome Desconhecido"}</Text>
                    </TouchableOpacity>
                ))
            ) : (
